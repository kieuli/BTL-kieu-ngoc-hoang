...
# check if name, password, name submitted
if(isset($_POST['email']) &&
   ...
   if (empty($name)) {
      ...
      */
   	  ...
   	  # checking the database if the name is taken
   	  $sql = "SELECT name 
   	          FROM users
   	          WHERE name=?";
      ...
      if($stmt->rowCount() > 0){
      		...
      		if($error === 0){
				...
				**/
				if (in_array($img_ex_lc, $allowed_exs)) {
					/** 
					 renaming the image with user's name
					 like: name.$img_ex_lc
					**/
      	...
      	if (isset($new_img_name)) {
            ...
            $sql = "INSERT INTO users
                    (email, name, password, avata)
                    VALUES (?,?,?,?)";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$email, $name, $password, $new_img_name]);
      	}else {
            # inserting data into database
            $sql = "INSERT INTO users
                    (email, name, password)
